<template>
  <div class="space-y-6">
    <!-- Section 1: Transaction Type Configuration -->
    <div class="space-y-4">
          ...
          <div class="flex items-start gap-3">
            ...
            <div>
              <h4 class="text-sm font-medium text-amber-800 dark:text-amber-200">
                Standard CBV Transaction Types
              </h4>
              <p class="text-xs text-amber-600 dark:text-amber-300 mt-1">
                Select from the Core Business Vocabulary (CBV) standard transaction types
                like Purchase Order, Invoice, etc.
              </p>
            </div>
          </div>
        </div>
        ...
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400">
              Select which transaction types are permitted
            </p>
          </div>
      ...
      <div v-else-if="selectedTypeMode === 'custom'" class="space-y-4">
          ...
          <div class="flex items-start gap-3">
            ...
            <div>
              ...
              <p class="text-xs text-blue-600 dark:text-blue-300 mt-1">
                Provide a regular expression pattern to validate custom transaction type URIs.
              </p>
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <UInput
            ...
            placeholder="Enter regex pattern (e.g., ^urn:epcglobal:cbv:btt:.*)"
    ...
    <!-- Section 2: Transaction Value Configuration -->
    <div class="space-y-4">
      <div class="flex items-center gap-2">
        <UIcon
        ...
        />
        <h4 class="text-sm font-semibold text-gray-900 dark:text-gray-100">
          Transaction Value (bizTransaction)
        </h4>
      </div>
        ...
        >
          <div class="flex items-start gap-3">
            ...
            <div>
              ...
              <p class="text-xs text-blue-600 dark:text-blue-300 mt-1">
                Provide a regular expression pattern to validate the bizTransaction URI value.
              </p>
            </div>
          </div>
        </div>
    ...
    <ArrayCountConfigPanel
      ...
      title="Transaction List Array Constraints"
      description="Set minimum and maximum number of business transactions allowed in the list."
...
import type { FieldOption, BizTransactionListConfig } from "~/types/profile";
...
const emit = defineEmits<{
  (e: "update:bizTransactionConfig", value: BizTransactionListConfig): void;
}>();
...
const typeExamplePatterns = [
  {
    label: "CBV BTT URN",
    pattern: "^urn:epcglobal:cbv:btt:.*",
    description: "EPCIS CBV business transaction type URN",
  },
  {
    label: "Any URN",
    pattern: "^urn:.*",
...
const selectedTypeMode = ref<"standard" | "custom">(
  props.bizTransactionConfig?.typeMode || "standard"
);
const localSelectedTypes = ref<string[]>(
  props.bizTransactionConfig?.selectedTypes || []
);
const localCustomTypePattern = ref<string>(
  props.bizTransactionConfig?.customTypePattern || ""
);

const selectedValueMode = ref<"uri" | "custom">(
  props.bizTransactionConfig?.valueMode || "uri"
);
const localCustomValuePattern = ref<string>(
  props.bizTransactionConfig?.customValuePattern || ""
);
const localMinItems = ref<number | undefined>(
  props.bizTransactionConfig?.minItems
);
const localMaxItems = ref<number | undefined>(
  props.bizTransactionConfig?.maxItems
);
...
const emitUpdate = () => {
  const config: BizTransactionListConfig = {
  ...
  emit("update:bizTransactionConfig", config);
};
