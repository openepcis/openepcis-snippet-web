<template>
  <div class="space-y-4">
    ...
    <div class="flex items-center gap-2">
      <UIcon name="i-heroicons-bars-3-bottom-left" class="w-4 h-4 text-purple-500" />
      ...
      <div class="flex items-start gap-3">
        ...
        <div>
          <h4 class="text-sm font-medium text-purple-800 dark:text-purple-200">
            {{ description || 'Set minimum and maximum number of items allowed in the array.' }}
          </h4>
...
const presets = [
  ...
  { label: "At least 1", min: 1, max: undefined },
  { label: "At most 1", min: undefined, max: 1 },
  { label: "1-5", min: 1, max: 5 },
  { label: "1-10", min: 1, max: 10 },
  { label: "No limit", min: undefined, max: undefined },
];
...
const isPresetActive = (preset: { min?: number; max?: number }) => {
  return localMinItems.value === preset.min && localMaxItems.value === preset.max;
};
...
const emitUpdate = () => {
  ...
  const minVal = localMinItems.value && !isNaN(localMinItems.value) ? localMinItems.value : undefined;
  const maxVal = localMaxItems.value && !isNaN(localMaxItems.value) ? localMaxItems.value : undefined;
